...
/*
 * Implements a standalone CLI app that opens Voidly from a remote terminal.
 ...
 *  - In external terminals for WSL this calls Voidly on the Windows side.
 *    The Voidly desktop executable path is passed in env VSCODE_CLIENT_COMMAND.
 */
...
export async function main(desc: ProductDescription, args: string[]): Promise<void> {
	if (!cliPipe && !cliCommand) {
		console.log('Command is only available in WSL or inside a Voidly terminal.');
		return;
	}
...
function fatal(message: string, err: any): void {
	console.error('Unable to connect to Voidly server: ' + message);
	console.error(err);
	process.exit(1);
}
