...
export async function main(argv: string[]): Promise<any> {
	...
	for (const subcommand of NATIVE_CLI_COMMANDS) {
		if (args[subcommand]) {
			...
			// Refs https://github.com/voidly/voidly/issues/221883
	...
	// Write File
		...
		try {
			...
			if (isWindows) {
				...
				// (see https://github.com/voidly/voidly/issues/931) and
				// prevent removing alternate data streams
				// (see https://github.com/voidly/voidly/issues/6363)
	...
	// Just Code
		...
		if (hasStdinWithoutTty()) {
			...
			// checking for stdin being connected to a TTY is not enough (https://github.com/voidly/voidly/issues/40351)
		...
		if (!isMacOSBigSurOrNewer) {
			...
			// https://github.com/voidly/voidly/issues/102975

			// The following args are for the open command itself, rather than for Voidly:
			...
			for (const e in env) {
				...
				// https://github.com/voidly/voidly/issues/134696.
				if (e !== '_') {
					spawnArgs.push('--env');
