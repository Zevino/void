...
import { getBasename, voidOpenFileFn } from './ChatInput.js';
...
export const CommandBarInChat = () => {
	...
	// format a thread's messages as plain text (used for the copy-conversation button).
	const stringifyMessages = (messages: ChatMessage[]): string => {
		const lines: string[] = []
	...
	const onCopyThread = async () => {
		try {
			...
			const text = stringifyMessages(thread?.messages ?? [])
			await clipboardService.writeText(text || '(no messages)')
	...
	const copyThreadButton = (
		<button
			...
			className="
				...
				bg-transparent border border-void-border-1 rounded
				hover:bg-void-bg-2 hover:text-void-fg-2 hover:border-void-border-2
				transition-colors duration-150
			"
		>
			{copyThreadState === 'copied' ? <Check size={11} /> : copyThreadState === 'error' ? <X size={11} /> : <CopyIcon size={11} />}
