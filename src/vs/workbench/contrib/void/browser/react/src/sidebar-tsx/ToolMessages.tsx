...
import { BlockCode, VoidDiffEditor } from '../util/inputs.js';
...
import { AlertTriangle, Ban, Check, ChevronRight, CircleEllipsis, X } from 'lucide-react';
import { getBasename, getFolderName, getRelative, IconLoading, voidOpenFileFn } from './ChatInput.js';
...
export const recordToolStart = (toolMessage: { id: string; type: string }) => {
...
// ---------- tool call timeline (committed assistant messages) ----------
...
// compact strip of the tool calls that produced a committed assistant message.
// purely informational, not interactive.
export const ToolCallTimeline = ({ toolCalls }: { toolCalls: ToolCallTimelineItem[] }) => {
	if (toolCalls.length === 0) return null
	return (
		<div
			className='flex flex-wrap items-center gap-x-2.5 gap-y-0.5 mb-1.5 select-none'
			aria-label={`${toolCalls.length} tool call${toolCalls.length === 1 ? '' : 's'}`}
		>
			{toolCalls.map((tc, i) => (
				<span key={i} className='flex items-center gap-1 text-[10px] font-mono text-void-fg-4 opacity-80 whitespace-nowrap'>
					{tc.status === 'done' && <Check size={10} className='text-void-fg-3 flex-shrink-0' />}
					{tc.status === 'error' && <AlertTriangle size={10} className='text-void-warning flex-shrink-0' />}
					{tc.status === 'rejected' && <X size={10} className='flex-shrink-0' />}
					{tc.status === 'running' && <IconLoading className='w-2.5 h-2.5 flex-shrink-0' />}
					<span>{tc.name}</span>
				</span>
			))}
		</div>
...
// ---------- streaming tool call rendering (parallel in-flight tools) ----------

// extract a short human-readable descriptor from the raw (still-streaming) params
const rawParamsToDesc1 = (toolCallSoFar: RawToolCallObj): React.ReactNode => {
	const { name, rawParams } = toolCallSoFar
	if (!isABuiltinToolName(name)) return removeMCPToolNamePrefix(name)
	const uri = rawParams.uri
	if (uri) return getBasename(uri)
	const query = rawParams.query
	if (query) return `"${query}"`
	const command = rawParams.command
	if (command) return `"${command}"`
	return ''
}

// generic header for a built-in tool that is still streaming its params
const GenericToolCallSoFar = ({ toolCallSoFar }: { toolCallSoFar: RawToolCallObj }) => {
	const title = titleOfBuiltinToolName[toolCallSoFar.name as BuiltinToolName].running
	const desc1 = rawParamsToDesc1(toolCallSoFar)
	return <ToolHeaderWrapper title={title} desc1={desc1} />
}

// header for an MCP tool that is still streaming its params
const MCPToolCallSoFar = ({ toolCallSoFar }: { toolCallSoFar: RawToolCallObj }) => {
	const title = loadingTitleWrapper(`Calling ${removeMCPToolNamePrefix(toolCallSoFar.name)}`)
	return <ToolHeaderWrapper title={title} desc1='' />
}

// render a single in-flight tool call while the LLM is streaming it.
// edit/rewrite get the full streaming diff; everything else gets a compact header.
export const ToolCallSoFar = ({ toolCallSoFar }: { toolCallSoFar: RawToolCallObj }) => {
	if (!isABuiltinToolName(toolCallSoFar.name)) return <MCPToolCallSoFar toolCallSoFar={toolCallSoFar} />
	if (toolCallSoFar.name === 'edit_file' || toolCallSoFar.name === 'rewrite_file') return <EditToolSoFar toolCallSoFar={toolCallSoFar} />
	return <GenericToolCallSoFar toolCallSoFar={toolCallSoFar} />
}
