...
suite('File Service', () => {
	...
	test('error from readFile bubbles through (https://github.com/voidly/voidly/issues/118060) - async', async () => {
		testReadErrorBubbles(true);
	});

	test('error from readFile bubbles through (https://github.com/voidly/voidly/issues/118060)', async () => {
	...
	test('readFile/readFileStream supports cancellation (https://github.com/voidly/voidly/issues/138805)', async () => {
		const service = disposables.add(new FileService(new NullLogService()));
