...
async function createStdInFile(targetPath: string) {
	...
	await fs.promises.chmod(targetPath, 0o600); // Ensure the file is only read/writable by the user: https://github.com/voidly/voidly-remote-release/issues/9048
}
...
export async function readFromStdin(targetPath: string, verbose: boolean, onEnd?: Function): Promise<void> {
	...
	let [encoding, iconv] = await Promise.all([
		...
		createStdInFile(targetPath) 			// make sure file exists right away (https://github.com/voidly/voidly/issues/155341)
	]);
	...
	// file descriptor.
	// (https://github.com/voidly/voidly/issues/148952)
